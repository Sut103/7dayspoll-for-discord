...
func GetI18n(lang discordgo.Locale) I18n {
	return I18n{
		...
		PollEndTargetNotFound: getPollEndTargetNotFound(lang),
	}
}
...
func getPollEndTargetNotFound(lang discordgo.Locale) string {
	pollEndTargetNotFound := map[discordgo.Locale]string{
		discordgo.EnglishUS: "That poll message could not be found in this channel.",
		discordgo.Japanese:  "このチャンネルでその投票メッセージを見つけられませんでした。",
	}
	name, ok := pollEndTargetNotFound[lang]
	if !ok {
		return pollEndTargetNotFound[discordgo.EnglishUS]
	}
	return name
}
